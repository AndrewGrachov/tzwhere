...
var init = function(tzWorldFile, initOptions){
...
var constructShortcuts = function (tzWorldFile) {
  ...
  if ((timezoneNamesToPolygons === null) || (timezoneLongitudeShortcuts === null)) {
    ...
    if (options.forceCache) {
      console.time('tzwhereCacheRead');
      var shortcuts = JSON.parse(fs.readFileSync(constructedShortcutFilePath));
      timezoneLongitudeShortcuts = shortcuts.timezoneLongitudeShortcuts;
      timezoneLatitudeShortcuts = shortcuts.timezoneLatitudeShortcuts;
      timezoneNamesToPolygons = shortcuts.timezoneNamesToPolygons;
      console.timeEnd('tzwhereCacheRead');
      ...
      fs.writeFileSync(path.join(__dirname + '/shortcuts.json'), JSON.stringify(shortcutObj));
...
var tzNameAt = function (latitude, longitude) {
  ...
  console.log('POSSIBLE TIMEZONES LENGTH:', possibleTimezones);
